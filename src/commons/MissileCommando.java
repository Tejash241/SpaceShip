...
/* A simple rectangular base.  This object just sits around waiting to explode. */
class Base extends Thing {
    ...
    public boolean hit (Missile m){
    	return (m.X >= X && m.X <= (X + W)
    			&& m.Y >= Y && m.Y <= (Y + H)); //get the pixel addresses of the missile and check whether it falls within base
    }
...
class Missile extends Thing
{
    ...
    private int X2 = 0;
    private int Y2 = 0;
    //the above x,y coordinates are the coordinates of the start and end points of the missile
    
    private int V = 0;
    private float m = 0; //m is the slope of the line
    private float b = 0;
    private float x = 0;
    private float y = 0;
    ...
    Missile (MissileCommando parent, Color color, int x1, int y1, int x2, int y2, int v)   {
		this.parent = parent;
		//set X and Y of Thing class to 0, here they represent the CURRENT coordinates of the missile
		X = X1;
		Y = Y1;
		this.color = color;
    ...
    public void run () {
		...
		m = (float) (Y2 - Y1) / (X2 - X1); //obtain slope by two-point form
		b = Y1 - (m*X1);
		x = X1;
		y = Y1;
	
		/* 10% chance this missile will replicate. */
		replicate = Math.random () > 0.90; //Math.random() will return a double value >= 0.0 and < 1.0
		if (replicate){
		    color = Color.magenta;
		}
...
/* Generic explosion which draws a circle that grows and shrinks. The explosion is drawn at X,Y and has a maximum size of S. */
...
class Explosion extends Thing{
    ...
    Explosion (MissileCommando parent, Color color, int x, int y, int s)   {
		...
		S = s;
		start (); //causes this thread to start. The JVM calls the "run" method of this thread
    }
...
class Message extends Thing
{
    ...
    Message (MissileCommando parent, String message, int x, int y, int blinks)    {
    	this (parent, message, x, y, blinks, 500); // if not mentioned, delay = 0.5 sec
    }
...
public class MissileCommando extends java.applet.Applet implements Runnable{
    ...
    public void init (){
    	setSize (WORLD_WIDTH + SCORE_WIDTH, WORLD_HEIGHT); //setSize function should always be at the start or else its equivalent to the resize function which 
    	//starts off the applet with the default screen size and then resizes it to the specified size
    	font = new Font ("TimesRoman", Font.BOLD, 24);
		fontMetrics = getFontMetrics (font);
		System.out.println(fontMetrics.toString());
		setFont (font);
    }
    ...
    void getSounds (){
		...
		/* background music is disabled since the sound file is too large. */
    ...
    void createBase (){
		...
		for (i = 0, x = BASE_SPACING; i < BASES; i++, x += BASE_WIDTH + BASE_SPACING){
		    ...
		    if (!found) {
				things.addElement (new Base (this, Color.blue, x, WORLD_HEIGHT - BASE_HEIGHT - 1,BASE_WIDTH,BASE_HEIGHT));
				break;
		    }
		}
    }
    ...
    public boolean handleEvent (Event e)    {
		if (e.id == Event.MOUSE_DOWN) /* mouse click */{
		    if (playing && shotCount > 0 && e.x >= 0 && e.x <= WORLD_WIDTH	&& e.y >= 0 && e.y <= WORLD_HEIGHT){
